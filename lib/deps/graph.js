...
Graph.prototype.addNode = function(id, attrs) {
  this.nodes.set(id, new Node(this, id));
  if( attrs ) {
    for( k in attrs ) {
      this.nodes.get(id).set( k, attrs[k] );
    }
  }
 ...
 * @api public
 */
Graph.prototype.getNode = function(id, explored={}) {
  if (this.nodes.get(id)) {
    return this.nodes.get(id);
  }

  found = null;
  Array.from(this.clusters.entries()).some(([clusterId, cluster]) => {
    if(!explored[clusterId]) {
      explored[clusterId] = true;
      found = cluster.getNode(id, explored);
      if (found) {
	return true;
      }
    }
  ...
  let myParent = this.relativeGraph;
  while (myParent) {
    if (!explored[myParent.id]) {
      explored[myParent.id] = true;
      const node = myParent.getNode(id, explored);
      if (node) {
      	return node;
      }
    }
    myParent = myParent.relativeGraph;
  }
...
Graph.prototype.addEdge = function(nodeOne, nodeTwo, attrs) {
  ...
  if( typeof(nodeOne) == 'string' ) {
    _nodeOne = this.getNode(nodeOne);
    if( _nodeOne == null ) {
      _nodeOne = this.addNode( nodeOne );
    }
  }
  if( typeof(nodeTwo) == 'string' ) {
    _nodeTwo = this.getNode(nodeTwo);
    if( _nodeTwo == null ) {
...
Graph.prototype.to_dot = function() {
  ...
  this.clusters.forEach((cluster, clusterId) => {
    dotScript = dotScript + cluster.to_dot() + '\n';
  })
  
  // Each nodes
  this.nodes.forEach((node, id) => {
      dotScript = dotScript + '  ' + node.to_dot() + ';\n'
  })
